...
enum AntialiasedElement {
    ...
    ,
    aeScatters = 0x0080  ///< <tt>0x0080</tt> Scatter symbols of plottables (excluding scatter symbols of type
                         ///< ssPixmap)
    ,
...
enum PlottingHint {
    ...
    phFastPolylines = 0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster method. This
                             ...
                             ///<                joins, thus is most effective for pen sizes larger than 1. It
                             ///<                is only used for solid line pens.
    ,
    phImmediateRefresh =
    ...
    ,
    phCacheLabels = 0x004  ///< <tt>0x004</tt> axis (tick) labels will be cached as pixmaps, increasing replot
                           ///< performance.
};
...
enum Interaction {
    ...
    ,
    iMultiSelect = 0x004  ///< <tt>0x004</tt> The user can select multiple objects by holding the modifier set
...
enum SelectionType {
    ...
    ,
    stWhole  ///< Selection behaves like \ref stMultipleDataRanges, but if there are any data points selected,
             ///< the entire plottable is drawn as selected.
    ,
...
class QCP_LIB_DECL QCPPainter : public QPainter {
    ...
    */
    enum PainterMode {
        ...
        ,
        pmNoCaching = 0x02  ///< <tt>0x02</tt> Mode for all sorts of exports (e.g. PNG, PDF,...). For example,
                            ///< this prevents using cached pixmap labels
        ,
        pmNonCosmetic =
...
class QCPLabelPainterPrivate {
   ...
   protected:
    ...
    QColor mColor;
    int mPadding;
    double mRotation;  // this is the rotation applied uniformly to all labels, not the heterogeneous rotation
                       // in amCircularRotated mode
...
#endif
    void setTickOrigin(double origin);  // hides base class method but calls baseclass implementation ("using"
...
class QCP_LIB_DECL QCPScatterStyle {
    ...
    */
    enum ScatterShape {
        ...
        ,
        ssDot  ///< \enumimage{ssDot.png} a single pixel (use \ref ssDisc or \ref ssCircle if you want a round
               ///< shape with a certain radius)
        ,
    ...
    */
    enum PositionType {
        ptAbsolute  ///< Static positioning in pixels, starting from the top left corner of the
                    ///< viewport/widget.
        ,
        ptViewportRatio  ///< Static positioning given by a fraction of the viewport size. For example, if you
                         ///< call setCoords(0, 0), the position will be at the top left corner of the
                         ///< viewport/widget. setCoords(1, 1) will be at the bottom right corner,
                         ///< setCoords(0.5, 0) will be horizontally centered and vertically at the top of the
                         ///< viewport/widget, etc.
        ,
        ptAxisRectRatio  ///< Static positioning given by a fraction of the axis rect size (see \ref
                         ...
                         ///< the top of the axis rect, etc. You can also go beyond the axis rect by providing
...
class QCP_LIB_DECL QCustomPlot : public QWidget {
    ...
    */
    enum RefreshPriority {
        ...
        ,
        rpQueuedRefresh  ///< Replots immediately, but queues the widget repaint, by calling QWidget::update()
                         ///< after the replot. This way multiple redundant widget repaints can be avoided.
        ,
...
void QCPAbstractPlottable1D<DataType>::getDataSegments(QList<QCPDataRange> &selectedSegments,
    ...
    if (mSelectable == QCP::stWhole)  // stWhole selection type draws the entire plottable with selected style
                                      // if mSelection isn't empty
    {
        if (selected())
    ...
    */
    enum GradientPreset {
        ...
        gpCold  ///< Continuous lightness from black over icey colors to white (suited for non-biased data
                ///< representation)
        ,
        gpNight  ///< Continuous lightness from black over weak blueish colors to white (suited for non-biased
                 ///< data representation)
        ,
